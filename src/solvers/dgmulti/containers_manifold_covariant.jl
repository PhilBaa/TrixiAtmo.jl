@muladd begin
    ...
    function init_auxiliary_node_variables!(aux_values, mesh::DGMultiMesh,
        ...
        for element in eachelement(mesh, dg)
            ...
            vertices = map(i -> getindex.([VX, VY, VZ], i), 1:length(VX))
    ...
    @inline function calc_basis_covariant(v1, v2, v3, v4, v5, v6, r, s, t, _,
                                          ::GlobalCartesianCoordinates)

        # Construct a bilinear mapping based on the six corner vertices
        xe = 0.25f0 * ((-(r + s) * v1 + (1 + r) * v2 +
                         (1 + s) * v3) * (1 - t) +
                       (-(r + s) * v4 + (1 + r) * v5 +
                         (1 + s) * v6) * (1 + t))
        ...
        dxeds = 0.25f0 *
                ((-v1 + v3) * (1 - t) +
                 (-v4 + v6) * (1 + t))
        dxedt = 0.25f0 *
                (-(-(r + s) * v1 + (1 + r) * v2 + (1 + s) * v3) +
                  (-(r + s) * v4 + (1 + r) * v5 + (1 + s) * v6))
        
        # Use product/quotient rule on the projection
        norm_xe = norm(xe)
        radius = norm(v1) + 0.5 * (1 + t) * (norm(v4) - norm(v1))
        dxdr = radius / norm_xe * (dxedr - dot(xe, dxedr) / norm_xe^2 * xe)
        dxds = radius / norm_xe * (dxeds - dot(xe, dxeds) / norm_xe^2 * xe)
        dxdt = radius / norm_xe * (dxedt - dot(xe, dxedt) / norm_xe^2 * xe) + 0.5 * (norm(v4) - norm(v1)) / norm_xe * xe
    ...
    function calc_christoffel_symbols!(aux_values, mesh::DGMultiMesh,
                                       equations::AbstractCovariantEquations{3, 3}, dg,
                                       element)
        rd = dg.basis
        (; Vq, Drst) = rd
        Dr, Ds = Drst
